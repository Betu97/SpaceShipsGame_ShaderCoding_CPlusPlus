...
bool Shader::createShaderObject(unsigned int type, GLuint& handle, const std::string& code)
{
    ...
    //bind attributes
    glBindAttribLocation(program, VERTEX_ATTRIBUTE_LOCATION, "a_vertex");
    glBindAttribLocation(program, NORMAL_ATTRIBUTE_LOCATION, "a_normal");
    glBindAttribLocation(program, UV_ATTRIBUTE_LOCATION, "a_uv");
...
void Shader::init()
{
	...
	if(firsttime)
	{
        //any code which needs to loaded when shader first loaded
